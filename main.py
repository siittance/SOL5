...
def register():
    try:
        ...
        if not is_strong_password(password):
            flash(
                'Пароль слишком слабый. Убедитесь, что он содержит как минимум 12 символов, включая заглавные и строчные буквы, цифры и специальные символы.',
                'danger')
            return redirect(url_for('index'))
...
def send_eth():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            tx_hash = contract.functions.pay(amount).transact({
                'from': public_key,
            })
...
def create_ad():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            price_eth = int(request.form['price'])
            price_wei = int(price_eth)
            tx_hash = contract.functions.createAd(id_estate - 1, price_wei).transact({
                'from': public_key
            })
...
def buy_estate():
    ...
    if request.method == 'POST':
        try:
            ...
            if 0 <= id_ad - 1 < len(ads):
                tx_hash = contract.functions.BuyEstate(id_ad - 1).transact({
                    'from': public_key,
                })
                flash(f"Транзакция {tx_hash.hex()} отправлена для покупки недвижимости", 'success')
                return redirect(url_for('dashboard'))
            else:
                flash('Ошибка: неверный ID недвижимости', 'danger')
